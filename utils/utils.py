...
def make_meshgrid(x, y, h=0.02):
    x_min, x_max = x.min() - ((np.abs(x.min()) + 0.1) * 0.1), x.max() + (
        np.abs(x.max()) * 0.1
    )
    y_min, y_max = y.min() - ((np.abs(y.min()) + 0.1) * 0.1), y.max() + (
        np.abs(y.max()) * 0.1
    )
...
def get_fingerprints(
    ...
    feature_matrix = pd.DataFrame(
        np.zeros((data.shape[0], bitSize_circular)), dtype=int
    )
    for i in tqdm(range(data.shape[0])):
        feature_matrix.iloc[i, :] = np.array(
            AllChem.GetMorganFingerprintAsBitVect(
                Chem.MolFromSmiles(data.smiles.iloc[i]),
                morgan_radius,
                nBits=bitSize_circular,
            )
        )
...
def evaluate(model, loader, dictionary):
    ...
    for input_seq, output_seq in iter(loader):
        ...
        pred = model(input_seq, output_seq, 0.0)
